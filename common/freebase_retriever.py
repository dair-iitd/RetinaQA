...
fb_domain_dict = json.load(open(file=domain_dict_file_path, mode="r"))
...
class FreebaseRetriever(KBRetriever):
    ...
    def __init__(self, cache_dir=freebase_cache_dir, freebase_addr=freebase_addr, freebase_port=freebase_port):
        super().__init__()
        print(freebase_cache_dir, freebase_addr)
        ...
        if os.path.isfile(fb_roles_file_path):
            self.roles = read_tsv_as_list(fb_roles_file_path, sep=' ')
            self.predicate_range_cache = dict()
            for t in self.roles:
                self.predicate_range_cache[t[1]] = t[2]

        if os.path.isfile(reverse_properties_file_path):
            pairs = read_tsv_as_list(reverse_properties_file_path)
